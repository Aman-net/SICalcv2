...
import { fmtINR, fmtDate, fmtDateFromTimestamp, buildShareText } from './calc';
...
async function doShare(text: string, onCopied?: () => void) {
  try { await navigator.clipboard.writeText(text); onCopied?.(); } catch { /* ignore */ }
  if (navigator.share) {
    try {
...
function BatchCard({ batch, isOpen, onToggle, onDelete, onShare }: BatchCardProps) {
        ...
        >
          <div className="flex-1 min-w-0">
            ...
            <div className="mt-1">
              ...
              {batch.entries.length} loan{batch.entries.length !== 1 ? 's' : ''}
              </span>
            </div>
          </div>
          <div className="shrink-0 text-right min-w-[76px]">
            <p className="text-[8px] font-bold text-slate-300 uppercase tracking-widest leading-none mb-0.5">Interest</p>
            <p className="text-sm font-bold text-indigo-500 leading-none">{fmtINR(batch.totalInterest)}</p>
          </div>
          <div className="shrink-0 text-right">
            <p className="text-[9px] font-bold text-slate-300 uppercase tracking-widest leading-none mb-0.5">Total Due</p>
            <p className="text-sm font-extrabold text-emerald-600 leading-none">{fmtINR(batch.grandTotal)}</p>
          </div>
          ...
          <div className="border-t border-slate-100">
            <div className="px-3 py-2 space-y-2">
              {batch.entries.map(e => {
                ...
                return (
                  <div key={e.id} className="bg-slate-50 rounded-xl overflow-hidden">
                    <div className="flex items-center gap-3 px-3 pt-2.5 pb-2">
                      <div className="flex-1 min-w-0">
                        <p className="text-[8px] font-bold text-slate-300 uppercase tracking-widest leading-none mb-0.5">Principal</p>
                        <p className="text-[17px] font-bold text-slate-800 tracking-tight truncate">{fmtINR(e.principal)}</p>
                      </div>
                      <div className="shrink-0 text-right">
                        <p className="text-[8px] font-bold text-slate-300 uppercase tracking-widest leading-none mb-0.5">Interest</p>
                        <p className="text-sm font-bold text-indigo-600 leading-none">{fmtINR(e.interest)}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-1.5 px-3 pb-2.5 text-[11px] text-slate-400">
                      <span className="bg-white text-slate-500 text-[10px] font-semibold px-1.5 py-0.5 rounded-full shrink-0 border border-slate-100">
                        {e.ratePerMonth}%/mo
                      </span>
                      <span className="truncate">{fmtDate(e.startDate)}</span>
                      <span className="text-slate-300 shrink-0">→</span>
                      <span className="truncate">{fmtDate(e.endDate)}</span>
                      <span className={`ml-auto shrink-0 text-[10px] font-bold px-1.5 py-0.5 rounded-full ${
                        ...
                        {minBilled ? `${e.days}d→30d` : `${e.days}d`}
                      </span>
                    </div>
                  </div>
                );
      ...
      {batches.map(b => {
        ...
        return (
          <BatchCard
            ...
            onShare={() => doShare(buildShareText(b), () => showToast('Copied ✓'))}
          />
        );
      })}
      {/* ── Toast notification ── */}
